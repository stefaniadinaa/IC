  ...
  public void remove(Interval toRemove) {
    ...
    while (iter.hasNext()) {
      ...
      if (current.getMin() > toRemove.getMax())
        break;
      if (current.replaceIfOverlapping((toRemove))) {
        Interval lowerPart = current.below(toRemove.getMin());
        Interval upperPart = current.above(toRemove.getMax());
        iter.remove();
        if (lowerPart.addIfNonEmpty() > 0.0) {
          iter.add(lowerPart);
        }
        if (upperPart.addIfNonEmpty() > 0.0) {
          iter.add(upperPart);
        }
      }
    }
